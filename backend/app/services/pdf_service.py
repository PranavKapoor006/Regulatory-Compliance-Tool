...
def _needs_ocr(pages: List[dict]) -> bool:
    ...
    return _readability_score(combined) < 120
...
def _extract_with_ocr(path: Path) -> Tuple[str, List[dict]]:
    ...
    try:
        for index, page in enumerate(doc, start=1):
            ...
            pages.append(result)
            header = f"--- Page {index} | method=ocr | rotation={result.get('rotation', 0)} | psm={result.get('psm', '')} | score={result.get('score', 0)} ---"
...
def extract_pdf_text(path: Path) -> Tuple[str, List[dict]]:
    ...
    try:
        ...
        try:
            for index, page in enumerate(doc, start=1):
                ...
                pages.append({"page": index, "text": text, "method": "native", "score": _readability_score(text)})
    ...
    if _needs_ocr(pages):
        text, ocr_pages = _extract_with_ocr(path)
